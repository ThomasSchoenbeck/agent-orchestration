...
**Still pending from the original file:** Bug 8, Bug 9 (B/C/D), Bug 10, Bug 11.
...
- **Bug 8** — agent name in `TaskLog` / `TaskComment`. *(spec: original task file)*
- **Bug 10** — status timeline from event log (needs Bug 8).
- **Bug 11** — provider form models table.
These don't block Phase 1 and can run alongside it.

### Phase 1 — Role/capability core *(the gate)*
- **Feature 3** in full: remove `Task.Type` + `TaskTypes`; add `Capabilities`; `review_role` + capability-gated `AWAITING_REVIEW`/`AWAITING_MERGE` routing (absorbs Bug 9 Part A); live `/api/meta/task-roles`; planner `bootstrap_project` + `Project.Description`; update `tools/plan.go`; seed seven roles.
- **Bug 9 B/C/D** alongside: executor submits review verdict; timeout requeue `REVIEWING → AWAITING_REVIEW`; `AWAITING_REVISION → AWAITING_REVIEW` re-entry.
- *Gate:* nothing downstream is correct until task routing is capability-driven.

### Phase 2 — Autonomy loop (Track A)
1. **Feature 2** — PR workflow: reviewer opens PR on approval; deployer (`handles_merge`) claims `AWAITING_MERGE`, reviews, approves/rejects; approval merges + deletes branch + completes; workdir cleanup. *(dep: Phase 1, Bug 9)*
...
If building one item at a time: **Bug 8 → Feature 3 (+ Bug 9 B/C/D) → Feature 2 → Feature 5 → Feature 4 → Feature 6 → Feature 7 → Feature 8 → Bug 10 → Bug 11 → AGENTS.md.** This keeps every step on already-built foundations and reaches a working autonomous loop (through Feature 4) before starting the agent-management track.
