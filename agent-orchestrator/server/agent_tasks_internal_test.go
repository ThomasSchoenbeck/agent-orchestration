...
// handleAgentDetail bare GET + PUT.
func TestAgentDetailGetAndPut(t *testing.T) {
	...
	if w := do(t, srv, http.MethodPut, "/api/agents/"+id, map[string]interface{}{"roles": []string{"worker", "reviewer"}}); w.Code != http.StatusOK {
...
// handleInternalProviders only accepts GET → POST is 405.
func TestInternalProvidersMethod(t *testing.T) {
	srv, _ := newAgentTestServer(t, "")
	if w := agentPost(t, srv, "/api/internal/providers", map[string]interface{}{}); w.Code != http.StatusMethodNotAllowed {
