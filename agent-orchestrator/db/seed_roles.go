...
func DefaultRoleDefinitions() []*RoleDefinition {
	...
	return []*RoleDefinition{
		mk("worker", "Worker",
			...
			nil,
			[]string{"read", "write", "diff", "tests", "comment", "run_subagent", "checkpoint_session"},
			noScope),
		mk("reviewer", "Reviewer",
			"Reviews work and owns the merge gate (code review + merge-review)",
			[]string{"handles_review", "handles_merge"},
			[]string{"read", "list", "comment", "run_subagent", "checkpoint_session"},
			noScope),
		mk("planner", "Planner",
			"Planning, architecture, orchestration, self-improvement analysis",
			[]string{"creates_tasks"},
			[]string{"bootstrap_project", "sync_scope", "complete_project", "create_work_package", "list_tasks", "read", "comment", "web_search", "run_subagent", "checkpoint_session"},
			nil),
		mk("researcher", "Researcher",
			"Web research, technology exploration, improvement proposals",
			[]string{"creates_tasks"},
			[]string{"web_search", "read", "comment", "create_work_package", "run_subagent", "checkpoint_session"},
			nil),
		mk("security", "Security",
			"Autonomous security analysis; claims role:security tasks from BACKLOG",
			nil,
			[]string{"read", "list", "run_command", "web_search", "comment", "run_subagent", "checkpoint_session"},
			nil),
		mk("deployer", "Deployer",
			"CI/CD, infrastructure, releases / deployment only",
			[]string{"handles_deploy"},
			[]string{"read", "run_command", "comment", "diff", "run_subagent", "checkpoint_session"},
			noScope),
		mk("designer", "Designer",
			"UI/UX design; can create implementation tasks for workers",
			[]string{"creates_tasks"},
			[]string{"read", "write", "comment", "create_work_package", "run_subagent", "checkpoint_session"},
			nil),
	}
}
