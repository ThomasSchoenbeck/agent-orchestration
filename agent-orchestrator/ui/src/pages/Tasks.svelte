<script>
  ...
  function renderDonut(logs) {
    const counts = {}
    for (const l of logs) {
      if (!hiddenTypes.has(l.event_type)) counts[l.event_type] = (counts[l.event_type] || 0) + 1
    }
...
<div class="flex h-full overflow-hidden">
  <div class="flex-1 flex flex-col overflow-hidden min-w-0">
      ...
      >
        ...
        <div class="flex items-center gap-2 flex-wrap">
          ...
          >
            ...
            {#each ["BACKLOG", "UNQUEUED", "AWAITING_INPUT", "DEVELOPING", "AWAITING_REVIEW", "REVIEWING", "AWAITING_REVISION", "AWAITING_MERGE", "MERGING", "COMPLETED", "FAILED"] as s}
          ...
          {#if selectedTaskId}
            ...
            >
            <button
              class="text-xs px-2 py-1 rounded transition-colors {showSidebar
                ? 'bg-accent text-white'
                : 'bg-surface-700 text-gray-400 hover:bg-surface-600'}"
              onclick={() => (showSidebar = !showSidebar)}>💬 Assistant</button
            >
          {/if}
          <button class="text-xs text-gray-500 hover:text-gray-300" onclick={refreshAll}
            >↻ Refresh</button
          >
          <button
            class="text-xs px-2 py-1 rounded bg-red-900/40 text-red-400 hover:bg-red-900/70 transition-colors"
            onclick={handleClearTaskLogs}>Clear logs</button
          >
          <button
            class="px-2 py-1 bg-accent hover:bg-accent-hover text-white text-xs rounded transition-colors"
            onclick={() => (showForm = !showForm)}>{showForm ? "Cancel" : "+ New"}</button
          >
        </div>
      </div>
      ...
      {#if showForm}
        ...
        >
            ...
            <div class="grid grid-cols-2 gap-2">
              {#if formAvailReqs.length > 0}
                <div>
                  ...
                  <div
                    class="bg-surface-700 border border-surface-500 rounded p-2 max-h-24 overflow-y-auto flex flex-col gap-0.5"
                  >
                    {#each formAvailReqs as req (req.id)}
                      <label
                        class="flex items-center gap-1.5 text-xs cursor-pointer hover:text-gray-200 text-gray-300"
                      >
                        <input
                          type="checkbox"
                          checked={formLinkedReqs.has(req.id)}
                          onchange={() => toggleFormReq(req.id)}
                          class="accent-accent"
                        />
                        {req.title}
                      </label>
                  ...
                  >
                    {#each formAvailFeats as feat (feat.id)}
                      ...
                      >
                        <input
                          type="checkbox"
                          checked={formLinkedFeats.has(feat.id)}
                          onchange={() => toggleFormFeat(feat.id)}
                          class="accent-accent"
                        />
                        {feat.title}
                      </label>
          ...
          <div class="flex items-center gap-2">
            ...
            <button
              type="submit"
              class="ml-2 px-3 py-1 bg-accent hover:bg-accent-hover text-white text-xs rounded transition-colors"
              >Create</button
            >
          </div>
        </form>
              ...
              <div class="flex items-start justify-between gap-2">
                <div class="flex-1 min-w-0">
                  <div class="flex items-center gap-1.5 flex-wrap">
                    ...
                    >
                    <span
                      class="text-[10px] px-1.5 py-0.5 rounded-full {statusColors[t.status] ||
    ...
    <!-- ── Log panel header ───────────────────────────────────────────────────── -->
    <div
      class="shrink-0 px-6 py-2 bg-surface-800 border-b border-surface-600 flex items-center justify-between"
    >
      <span class="text-xs font-semibold text-gray-400 uppercase tracking-wide"
        >Task Activity Logs</span
      >
    ...
    <div class="shrink-0 px-6 py-3 border-b border-surface-600 flex gap-4 items-start">
      ...
      <div class="flex-1 min-w-0">
        <div class="flex items-center gap-2 mb-1">
          ...
          {#each [{ l: "5 m", v: 5 }, { l: "1 hr", v: 60 }, { l: "1 day", v: 1440 }] as b}
            <button
              class="px-1.5 py-0.5 text-[10px] rounded transition-colors {bucketMinutes === b.v
                ? 'bg-accent text-white'
                : 'bg-surface-700 text-gray-400 hover:bg-surface-600'}"
      ...
      <div class="shrink-0 flex flex-col gap-0.5 pt-4 max-h-24 overflow-y-auto w-32">
        {#each Object.entries(TASK_COLORS) as [type, color]}
          <button
            class="flex items-center gap-1.5 text-[10px] text-left transition-opacity {hiddenTypes.has(
              type,
            )
              ? 'opacity-30'
              : ''}"
      ...
      {#each chartTypeFilter as filterType (filterType)}
        <span class="flex items-center gap-1 px-2 py-0.5 bg-accent/20 text-accent text-xs rounded">
          ...
          <button
            ...
            aria-label={`Remove ${filterType} filter`}>×</button
          >
        </span>
      {/each}
    </div>
              ...
              <tr class="border-t border-surface-700 hover:bg-surface-700/20">
                ...
                <td class="px-4 py-1">
                  <span
                    ...
                    style="background:{TASK_COLORS[l.event_type] || '#374151'}22;color:{TASK_COLORS[
                      l.event_type
                    ] || '#9ca3af'}">{l.event_type}</span
                  >
                </td>
