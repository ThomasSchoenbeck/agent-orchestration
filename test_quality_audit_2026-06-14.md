...
- [~] C6 — Paused by request at server 53.1% (baseline 41%). Eight verified batches: (1) settings/meta, (2) roles/skills extra paths, (3) requirements/features/conversations, (4) task dependencies/links/checklist, (5) task cost + pull requests, (6) agentcontext.go internal, (7) agents list/metrics options/task-type detail, (8) chat context builders + llm-chat validation. Each was run green by the user before moving on. Two real production bugs found and fixed (below).
  - **Remaining gap to 80% is mostly integration-bound, not unit-testable.** Deferred to a future session with shared fixtures: (a) the logs handlers (`handleLogs`/`handleTaskLogs`/`handleAgentLogs`) need the partitioned **log DB** wired in tests; (b) git-bound handlers (`handleProjectFile`/`Diff`/`Commits`/`Tree`/`InitRepo`/`Files`, `approvePR`) need a **real git-repo fixture**; (c) chat success paths and `handleWSChat` need a **live/fake LLM provider** through the router. Further pure unit-test gains (agent/skill/context/conversation detail paths) could reach ~65% without new infra.
  - **Real bug #2 found & fixed:** the `agentcontext.go` coverage test exposed that `writeReviewContext` never produced `review_thread.md`. It fetched comments via `ListTaskComments(taskID)` (= `ListComments(taskID, "")`, which filters `WHERE review_id IS NULL`) and then matched them against the review id (`c.ReviewID == latest.ID`) — impossible, since those comments have no review id. The review-thread agent-context feature was silently dead. Fixed by querying the review's comments directly via `ListComments(ctx, taskID, latest.ID)`.
  - **Real bug #1 found & fixed:** the task-checklist coverage test surfaced a production deadlock in `db.CloneChecklistIteration` (`db/task_checklist.go`). It held an open `QueryContext` rows cursor while issuing `ExecContext` INSERTs inside the `rows.Next()` loop; on the single-connection SQLite pool the insert blocked forever waiting for the connection the cursor still held. It would hang the server whenever the "new checklist iteration" feature ran on a checklist that had items. Fixed by materializing the rows and closing the cursor before inserting. A scan found no other instances of this pattern in `db/`. This is precisely the class of bug that slipped through before: the handler had no test exercising it with items present.
